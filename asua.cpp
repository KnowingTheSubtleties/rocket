...
// 将向量保存到文本文件的函数实现
void saveVector_double(const vector<vector<double>>& vector, const string& filename) {
...
vector<double> Acce_g(rocket& temp) {
	...
	double N_x = a / sqrt(1 - e2 * sin(fai) * sin(fai)) * cos(fai) * cos(lambda);
	double N_y = a / sqrt(1 - e2 * sin(fai) * sin(fai)) * cos(fai) * sin(lambda);
	double N_z = a * (1 - e2) / sqrt(1 - e2 * sin(fai) * sin(fai)) * sin(fai);
	vector<double> N = { N_x,N_y,N_z };

	// 更新rocket的Height
	temp.Height = r - Acce_count(N);
...
int main() {
		...
		for (int k = 0; k < 15; k++) {
			for (int j = 0; j < 10; j++) {
				for (int i = 0; i < 50; i++) { // 1s
					...
					test.xyz_fs_storage.push_back(test.xyz_fs);
	...
	saveVector_double(test.xyz_fs_storage, "xyz.txt");
...
void rfun(int n, double yc[], double dy[]) {
	...
	if (test.work_time > 65.75 + 53.15 + 47.5) {
		dy[1] = 0;
	}
	else {
